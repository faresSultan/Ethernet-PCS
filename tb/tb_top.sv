module tb_top;
  ...
  bit flips;  // 1 if the code flips RD state, 0 if it holds
  int errors = 0;
  ...
  initial begin
    ...
    for (int i = 0; i < 32; i++) begin
      flips = (exp_minus[i] !== exp_plus[i]);

      din = i[4:0]; rd_in = 1'b0; #10;             // entered RD-
      check(i, 1'b0, exp_minus[i], flips ? 1'b1 : 1'b0);  // flip->+  hold->-

      din = i[4:0]; rd_in = 1'b1; #10;             // entered RD+
      check(i, 1'b1, exp_plus[i],  flips ? 1'b0 : 1'b1);  // flip->-  hold->+
    end
